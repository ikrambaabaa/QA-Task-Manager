...
test('[TC-111] Tri alphabétique des tâches de A à Z', async ({ page }) => {
  ...
  // Créer plusieurs tâches non triées ici
  await page.locator('input[placeholder="Titre de la tâche..."]').fill('Banane');
  await page.locator('button:has-text("➕ Ajouter")').click();
  await page.locator('input[placeholder="Titre de la tâche..."]').fill('Pomme');
  await page.locator('button:has-text("➕ Ajouter")').click();
  await page.locator('input[placeholder="Titre de la tâche..."]').fill('Orange');
  await page.locator('button:has-text("➕ Ajouter")').click();
  ...
  expect(taskTitles).toEqual(['Banane', 'Orange', 'Pomme']);
...
test('[TC-113] Annulation de la modification du titre d\'une tâche', async ({ page }) => {
  ...
  await expect(page.locator('[data-task-title]').first()).toHaveText('Tâche à modifier');
});
...
test('[TC-114] Tentative de modification d\'une tâche avec un titre vide', async ({ page }) => {
  ...
  // Vérifier à la place que l'action n'a pas eu lieu
  await expect(page.locator('[data-task-title]').first()).toHaveText('Tâche à modifier');
});
...
test('[TC-115] Suppression de toutes les tâches après confirmation', async ({ page }) => {
  ...
  await expect(page.locator('[data-task-title]')).toHaveCount(0);
});
...
test('[TC-116] Annulation de la suppression de toutes les tâches', async ({ page }) => {
  ...
  const taskCount = await page.locator('[data-task-title]').count();
